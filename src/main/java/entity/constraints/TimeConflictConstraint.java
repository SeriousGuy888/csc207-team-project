    ...
    public boolean isSatisfiedBy(Set<Section> chosen) {
        ...
        for (int i = 0; i < list.length; i++) {
            for (int j = i + 1; j < list.length; j++) {
                if (list[i].getMeetings().iterator().next().getSemester()
                        == list[j].getMeetings().iterator().next().getSemester()
                        && list[i].getOccupiedTime().doesIntersect(list[j].getOccupiedTime())) {
                    return false;
                }
            }
        }
