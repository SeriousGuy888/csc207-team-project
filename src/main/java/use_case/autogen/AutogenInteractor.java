...
public class AutogenInteractor implements AutogenInputBoundary {
    ...
    public void execute(AutogenInputData inputData) {
        try {
            ...
            final List<CourseOffering> offerings = dataAccess.getSelectedCourseOfferings(
                    inputData.getSelectedCourses());
            final Set<Section> lockedSections = inputData.getLockedSections();
            final WeeklyOccupancy blockedTimes = inputData.getBlockedTimes();

            // Get variable and constraints using helper functions
            final List<CourseVariable> variables = applyLock(offerings, lockedSections);
            final List<Constraint> constraints = buildConstraints(blockedTimes);

            final PotentialTimetable result = generateTimetable(variables, constraints);

            if (result.getSuccess()) {
                final AutogenOutputData outputData = new AutogenOutputData(result.getTimetable());
    ...
    /**
     * @param variables The course variables the timetable is generated for
     * @param constraints All the constraints to be met
    ...
    public PotentialTimetable generateTimetable(List<CourseVariable> variables, List<Constraint> constraints) {
        final Set<Section> initial = new HashSet<>();
    ...
    private PotentialTimetable timetableSearch(int index, List<CourseVariable> variables, List<Constraint>
        ...
        final CourseVariable courseVariable = variables.get(index);
        // Iterates through all possible candidates in variable's domain
        for (Section candidate : courseVariable.getDomain()) {
            // tries a possible assignment by adding a candidate to it
            final Set<Section> next = new HashSet<>(assignment);
            next.add(candidate);
            ...
            if (isConsistent(next, constraints)) {
                ...
                final PotentialTimetable result = timetableSearch(index + 1,
                        variables, constraints, next);
        ...
        return new PotentialTimetable(false, Set.of());
    }

    /**
     * Checks if constraints are met, using the
     * classes in the constraints package
     * @param assignment The timetable assignment
     *                   that is being checked for consistency
     * @param constraints The constraints that need to be satisfied
    ...
    private boolean isConsistent(Set<Section> assignment, List<Constraint> constraints) {
        for (Constraint c : constraints) {
            ...
            // if a single one is not met function returns false
    ...
    private List<CourseVariable> applyLock(List<CourseOffering> offerings, Set<Section> lockedSections ) {
        final List<CourseVariable> variables = new ArrayList<>();
        for (CourseOffering offering : offerings) {
            final Set<Section> allSections = offering.getAvailableSections();
